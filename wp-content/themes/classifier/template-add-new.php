...
global $current_user, $colabs_abbr;
...
if ( isset($current_requirement) && $current_requirement && get_option($colabs_abbr.'_enable_membership_packs') == 'true') {
	//if no membership, or if membership but not a membership that satisfies required memberships, or if membership expired
	if(!isset($current_membership->pack_type) || !isset($current_user->membership_expires) || (isset($current_membership->pack_type) && !stristr($current_membership->pack_type, 'required')) || (colabsthemes_days_between_dates($current_user->membership_expires) < 0) ) {
		wp_redirect( CL_MEMBERSHIP_PURCHASE_URL.'?membership=required&cat='.$current_requirement );
		exit;
	}
}
...
add_action('wp_head', 'colabs_ajax_addnew_js_header' );
...
jQuery(document).ready(function(){
  ...
  jQuery('#step2 #term_condition').change(function(){
    var $el   = jQuery(this),
        $btn  = jQuery('.btn2 :submit');
    if( $el.is(':checked') ) {
      $btn.fadeIn();
    } else {
      $btn.fadeOut();
