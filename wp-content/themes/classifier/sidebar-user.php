<div class="sidebar col3">
	<div class="listing-info listing-details">	
		<h4 class="widget-title"><?php _e('User Options', 'colabsthemes');?></h4>
		<ul class="users-listing">
		<?php if ( is_user_logged_in() ) : 
		$logout_url = wp_logout_url( home_url() );
		?>
			
			<li><a href="<?php echo COLABS_DASHBOARD_URL ?>"><?php _e('My Dashboard','colabsthemes')?></a></li>
			<li><a href="<?php echo COLABS_PROFILE_URL ?>"><?php _e('Edit Profile','colabsthemes')?></a></li>
	...
	<?php
		if(get_option('colabs_membership_ending_reminder_email') == 'true'):
			if(($membership)&&(get_option('colabs_enable_membership_packs') == 'true')):
				$packname = stripslashes($membership->pack_name);
				$expire = date_i18n( get_option('date_format').' '.get_option('time_format'),strtotime($current_user->membership_expires), get_option('gmt_offset') );
				$url = COLABS_MEMBERSHIP_BUY_URL;
				colabs_expired_membership_email($current_user->ID,$packname,$expire,$url);
			endif;
		endif;
	?>
	...
	<div class="widget account_statistic">
		<h4 class="widget-title"><?php _e('Account Statistic', 'colabsthemes');?></h4>
		<ul>
