    ...
    <div class="main-content col9">
        <header class="entry-header">
            ...
            <tbody>
							...
							$wp_query = new WP_Query(array('posts_per_page' => 10, 'post_type' => COLABS_POST_TYPE, 'post_status' => 'publish, pending, draft', 'author' => $current_user->ID, 'paged' => $paged));
                ...
                // it's a pending ad which gives us several possibilities
                  ...
                  if(get_post_meta($post->ID, 'expires', true))
                      $check_expire_date = get_post_meta($post->ID, 'expires', true);
                  else
                      $check_expire_date = get_post_meta($post->ID, 'colabs_sys_expire_date', true);
											
                  // current date is past the expires date so mark ad ended
                  if (strtotime(date('Y-m-d H:i:s')) > (strtotime($check_expire_date))) {
										$poststatus = '<h4 class="ad-status">';
                ...
                <tr class="even">
                  ...
                  <td>
                    ...
                    <div class="meta">
										<span class="folder"><?php echo get_the_term_list(get_the_id(), COLABS_TAX_CAT, '', ', ', ''); ?></span> | <span class="clock"><span><?php the_time(get_option('date_format'))?></span></span>
										</div>
                  </td>
                    ...
                    if ( $post->post_status == 'pending' && $postaction != 'ended' ) {
                      ...
                      if ( ($total_cost != 0) && (get_option('colabs_enable_paypal') != 'false') ) {
                        ...
                        echo '&mdash;';
											...
											echo '<a onclick="return confirm_before_delete();" href="' . COLABS_DASHBOARD_URL . '?aid=' . $post->ID . '&amp;action=delete" style="display: block;"><img src="'.get_template_directory_uri().'/images/cross.png" title="'.__('Delete Ad', 'colabsthemes').'" alt="'.__('Delete Ad', 'colabsthemes').'" border="0" /></a>';
                      ...
                      if ( get_option('colabs_allow_relist') == 'true' ) {
                        ...
                        if ( ($total_cost != 0) && get_post_meta($post->ID, 'colabs_totalcost', true) == '' ) {
                          ...
                          echo '<a href="' . COLABS_DASHBOARD_URL . '?aid=' . $post->ID . '&amp;action=freerenew">' . __('Relist Ad', 'colabsthemes') . '</a>';
                        }
                      } else {
                        echo '&mdash;';
                      }
                    } else { ?>
                      <?php if ( get_option('colabs_ad_edit') == true ) : ?>
											<a href="<?php echo COLABS_EDIT_URL; ?>?aid=<?php the_id(); ?>">
											<img src="<?php echo get_template_directory_uri(); ?>/images/edit.png" title="" alt="" border="0" /></a>&nbsp;&nbsp;
											<?php endif; ?>
                      <a onclick="return confirm_before_delete();" href="<?php echo COLABS_DASHBOARD_URL; ?>?aid=<?php the_id(); ?>&amp;action=delete" title="<?php _e('Delete Ad', 'colabsthemes'); ?>"><img src="<?php echo get_template_directory_uri(); ?>/images/cross.png" title="<?php _e('Delete Ad', 'colabsthemes'); ?>" alt="<?php _e('Delete Ad', 'colabsthemes'); ?>" border="0" /></a>&nbsp;&nbsp;
											<a href="<?php echo COLABS_DASHBOARD_URL; ?>?aid=<?php the_id(); ?>&amp;action=<?php echo $postaction; ?>"><img src="<?php echo get_template_directory_uri(); ?>/images/<?php echo $postimage; ?>" title="" alt="" border="0" /></a><br />
                      <?php if ( get_post_meta(get_the_id(), 'colabs_ad_sold', true) != 'true' ) : ?>
												<a class="ads-mark" href="<?php echo COLABS_DASHBOARD_URL; ?>?aid=<?php the_id(); ?>&amp;action=setSold"><?php _e('Mark Sold', 'colabsthemes'); ?></a>
                      <?php else : ?>
												<a class="ads-mark" href="<?php echo COLABS_DASHBOARD_URL; ?>?aid=<?php the_id(); ?>&amp;action=unsetSold"><?php _e('Unmark Sold', 'colabsthemes'); ?></a>
											<?php endif; ?>
                    <?php } ?>
                  </td>
                </tr>
