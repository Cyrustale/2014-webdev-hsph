...
class CoLabs_googlemaps extends WP_Widget {
   ...
   function CoLabs_googlemaps() {
	   $widget_ops = array('description' => 'Add your Google Maps for the sidebar with this widget. This widget will automatic disabled when on single ad page.' );
       parent::WP_Widget(false, __('Colorlabs - Google Maps', 'colabsthemes'),$widget_ops);      
   }
   ...
   // output the content of the widget
	function widget($args, $instance) {		
		...
		if(!is_singular('ad')){
			print $before_widget;
			if ($instance['title']) { print $before_title.$instance['title'].$after_title; }else {print $before_title.__('Our Map','colabsthemes').$after_title;}
			$mapcss = "
			#colabsmap {
	...
	function form($instance) {
		...
		?>
			<p>
			<label for="<?php print $this->get_field_id('title'); ?>"><?php _e('Title:', 'colabsthemes'); ?></label>
			...
			<p>
			<label for="<?php print $this->get_field_id('lat'); ?>"><?php _e('Latitude:', 'colabsthemes'); ?></label>
			...
			<p>
			<label for="<?php print $this->get_field_id('lng'); ?>"><?php _e('Longitude:', 'colabsthemes'); ?></label>
			...
			<p>
			<label for="<?php print $this->get_field_id('width'); ?>"><?php _e('Width:', 'colabsthemes'); ?></label>
			...
			<p>
			<label for="<?php print $this->get_field_id('height'); ?>"><?php _e('Height:', 'colabsthemes'); ?></label>
			...
			<p>
			<label for="<?php print $this->get_field_id('zoom'); ?>"><?php _e('Zoom Level: <small>(1-19)</small>', 'colabsthemes'); ?></label>
			...
			<p>
			<label for="<?php print $this->get_field_id('type'); ?>"><?php _e('Map Type:<br /><small>(ROADMAP, SATELLITE, HYBRID, TERRAIN)</small>', 'colabsthemes'); ?></label>
			...
			<p>
			<label for="<?php print $this->get_field_id('directionsto'); ?>"><?php _e('Address for directions:', 'colabsthemes'); ?></label>
			...
			<p>
			<label for="<?php print $this->get_field_id('content'); ?>"><?php _e('Info Bubble Content:', 'colabsthemes'); ?></label>
