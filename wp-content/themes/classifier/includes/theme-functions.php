...
define('COLABS_DASHBOARD_URL', get_bloginfo('url').'/'.get_option($colabs_abbr.'_dashboard_url').'/');
define('COLABS_PROFILE_URL', get_bloginfo('url').'/'.get_option($colabs_abbr.'_profile_url').'/');
define('COLABS_EDIT_URL', get_bloginfo('url').'/'.get_option($colabs_abbr.'_edit_item_url').'/');
define('COLABS_ADD_NEW_URL', get_bloginfo('url').'/'.get_option($colabs_abbr.'_add_new_url').'/');
define('COLABS_ADD_NEW_CONFIRM_URL', get_bloginfo('url').'/'.get_option($colabs_abbr.'_add_new_confirm_url').'/');
...
/* Post Format
/*-----------------------------------------------------------------------------------*/
add_theme_support( 'post-formats', array( 'audio', 'gallery', 'quote', 'chat' ) );

/* -------------------------------------------------*/
function wp_audio_player_head() {
	echo '<script type="text/javascript" src="'.get_template_directory_uri().'/includes/js/audio-player.js"></script>';
	echo '<script type="text/javascript">';
	echo 'AudioPlayer.setup("'.get_template_directory_uri().'/includes/js/player.swf", {';
	echo 'width: "100%",';
...
return "<p id=\"audioplayer_$no\">Alternative content</p><script type=\"text/javascript\">AudioPlayer.embed(\"audioplayer_$no\", {soundFile: \"$file\"});</script>";
}
add_shortcode('audioplayer', 'wp_audio_player');
...
?>
    ...
    <div class="post-meta">
      <span class="comment-count"><?php comments_popup_link(__('0', 'colabsthemes'), __('1', 'colabsthemes'), __('%', 'colabsthemes')); ?></span>
      <time><?php the_time( get_option( 'date_format' ) ); ?></time>
      <p><?php _e('Posted by','colabsthemes'); ?>&nbsp;<a href="#"><?php the_author_posts_link(); ?></a><br/><?php _e('In','colabsthemes'); ?>&nbsp;
			<?php if( is_post_type_archive('ad') ) {
			echo get_the_term_list( @$post->ID, COLABS_TAX_CAT, '' , ', ', '' );
			} else {
			the_category(', ');
			} ?>
			</p><?php edit_post_link( __('{ Edit }', 'colabsthemes'), '<span class="small">', '</span>' ); ?>
    </div>
...
function custom_search_join($join) {
	...
	if ( is_search() && isset($_GET['s']) ) {
		...
		$join .= " AND (x.taxonomy = '".COLABS_TAX_CAT."' OR 1=1) ";
		$join .= " AND (x.taxonomy = '".COLABS_TAX_LOC."' OR 1=1) ";
		...
		$location = $_GET['sloc'];
		if ( ! empty($location) ) {
			$included_ids[] = $location;
		}
		
		// Make included ids as a string
		if( count( $included_ids ) > 0 ) {
			$included_str = "'" . implode( "', '", $included_ids ) . "'";
			$join .= " AND x.term_id IN ($included_str) ";
		}
...
function search_query_filter($query) {
	if( $query->is_search() && $query->is_main_query() ) {
		...
		if( $_GET['sloc'] ) {
			$query_tax_args[] = array(
				'taxonomy' => COLABS_TAX_LOC,
				'field' => 'slug',
				'terms' => $_GET['sloc']
			);
		}
		
		$query->set('tax_query', $query_tax_args);

		// Search only post types ads
		if( isset( $_GET['action'] ) && 'advance_search' == $_GET['action'] ) {
			$query->set('post_type', 'ad');
		}

		// If search query is default text
		$var_q = stripslashes($_GET['s']);
		//empty the s parameter if set to default search text
		if ( __('What are you looking for?','colabsthemes') == $var_q ) {
			$var_q = '';
		...
		// Pagination
		// $query->set('paged', 2)
		// dbgx_trace_var( get_query_var( 'page' ) );

	}
}
add_action( 'pre_get_posts','search_query_filter' );
...
/**
 * Prevent AdBlock from blocking single ad page
 */
function adblock_blocker( $classes ) {
	$single_ad = array_search('single-ad', $classes);
	if( $single_ad ) {
		$classes[$single_ad] = 'single-listing';
	}

	return $classes;
}
add_filter( 'body_class', 'adblock_blocker' );
