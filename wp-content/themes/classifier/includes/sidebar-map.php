<?php
    ...
    if ( get_post_meta($post->ID, 'location', true) )
        ...
        $make_address = get_post_meta($post->ID, 'colabs_street', true) . '&nbsp;' . get_post_meta($post->ID, 'colabs_city', true) . '&nbsp;' . get_post_meta($post->ID, 'colabs_state', true) . '&nbsp;' . get_post_meta($post->ID, 'colabs_zipcode', true);
				
		if(!empty($make_address)):
    ?>
		
			<div class="listing-info listing-details">
				<h4><?php _e('Map','colabsthemes'); ?></h4>
...
function colabs_google_maps_js() {
?>
...
//<![CDATA[
    ...
    var noLuck = "<?php echo get_bloginfo('template_directory') ?>/images/gmaps-no-result.png";
    ...
    function codeAddress() {
        geocoder.geocode( { 'address': address }, function(results, status) {
          if (status == google.maps.GeocoderStatus.OK) {
            ...
            (function($) {
                $('#map').append('<div style="height:400px;background: url(' + noLuck + ') no-repeat center center;"><p style="padding:50px 0;text-align:center;"><?php _e('Sorry, the address could not be found.', 'colabsthemes') ?></p></div>');
                return false;
            })(jQuery);
          }
        });
