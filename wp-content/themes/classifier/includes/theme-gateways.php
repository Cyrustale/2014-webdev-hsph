...
function register_gateway_submenu_page() {
	$colabsgateway = add_submenu_page( 'colabsthemes', __('Payment Gateway Options','colabsthemes'), __('Gateways','colabsthemes'), 'manage_options', 'gateways', 'colabs_gateways' );
	add_action( "admin_print_scripts-$colabsgateway", 'colabs_load_only_gateway' );
}
/* colabs_admin_head()
--------------------------------------------------------------------------------*/
...
function colabs_gateways() {
	...
	<div class="wrap colabs_container">
        ...
        <div class="clear"></div>
				<?php colabs_update_options($options_gateways);?>
...
function colabs_update_options($options) {
    ...
    if (isset($_POST['submitted']) && $_POST['submitted'] == 'true') {
			...
			if ( get_option('colabs_tools_run_expiredcheck') == 'true' ) {
					...
					$toolsMessage = __('Ads Expired Check was executed.', 'colabsthemes');
			}

      echo '<div id="colabs-popup-save" class="colabs-save-popup" style="display:block;"><div class="colabs-save-save">'.__('Your settings have been saved.','colabsthemes'). ' ' . $toolsMessage . '</div></div>';
    }
	return $message;
}
