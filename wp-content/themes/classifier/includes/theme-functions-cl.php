...
function colabs_get_price_legacy($postid) {
  ...
  if(get_post_meta($postid, 'colabs_price', true)) {
    ...
    $currency_opts = get_option('colabs_curr_symbol_pos');
    $currency_symbol = get_option($colabs_abbr.'_curr_symbol');

    // uncomment the line below to change price format
    //$price_out = number_format($price_out, 2, ',', '.');

    if( $currency_opts == 'right' ){
      $price_out = "{$price_out}{$currency_symbol}";
    } elseif( $currency_opts == 'left_space' ) {
      $price_out = "{$currency_symbol} {$price_out}";
    } elseif( $currency_opts == 'right_space' ) {
      $price_out = "{$price_out} {$currency_symbol}";
    } else {
      $price_out = "{$currency_symbol}{$price_out}";
    }
...
function colabs_get_order_vals( $order_vals ) {
    ...
    $order_vals['return_url'] = COLABS_ADD_NEW_CONFIRM_URL . '?pid=' . get_post_meta( $order_vals['post_id'], 'colabs_sys_ad_conf_id', true ) . '&amp;aid=' . $order_vals['post_id'];
...
function colabs_get_order_pack_vals( $order_vals ) {
	...
	if ( isset( $pack->pack_name ) && stristr( $pack->pack_status, 'membership' ) ) {
		...
		$order_vals['return_url'] = get_permalink(get_option('colabs_membership_auth')) . '?oid=' . $order_vals['oid'];
...
function colabs_ajax_addnew_js_header() {
	$parentPosting = get_option('colabs_ad_parent_posting');
...
function colabs_update_listing() {
    ...
    if ( $post_id ) {
        ...
        $errmsg = '<div class="box-green"><b>' . __('Your ad has been successfully updated.','colabsthemes') . '</b> <a href="' . COLABS_DASHBOARD_URL . '">' . __('Return to my dashboard','colabsthemes') . '</a></div>';
                    ...
                    foreach ($results as $result) :
                        ...
                        if (!empty($post_meta_val))
                            if($result->field_type == "checkbox"){
                                ...
                                echo '<li id="'. $result->field_name .'"><strong>' . $result->field_label . ':</strong> ' . colabsthemes_make_clickable(implode(", ", $post_meta_val)) .'</li>'; // make_clickable is a WP function that auto hyperlinks urls}
                            }elseif($result->field_name != 'colabs_price' && $result->field_type != "text area"){
                                echo '<li id="'. $result->field_name .'"><strong>' . $result->field_label . ':</strong> ' . colabsthemes_make_clickable($post_meta_val) .'</li>'; // make_clickable is a WP function that auto hyperlinks urls
                            }
                    endforeach;
                }
        ...
        if ( in_array($result->field_name, $custom_field_keys) || in_array($result->field_name, $field_req) || $result->field_type == 'checkbox' ) :
            ...
            ?>
                <li id="list_<?php echo $result->field_name; ?>">
                    <div class="labelwrapper">
                    	<label><?php if ($result->field_tooltip) : ?><a href="#" tip="<?php esc_attr_e($result->field_tooltip); ?>" tabindex="999"><div class="helpico"></div></a><?php endif; ?><?php esc_html_e($result->field_label, 'colabsthemes');?>: <?php if ($result->field_req) echo '<span class="colour">*</span>' ?></label><br />
            ...
            ?>
                <li id="list_<?php echo $result->field_name; ?>">
                    ...
                    <div class="clr"></div>
                    <textarea rows="4" cols="23" class="<?php if ($result->field_req) echo ' required'; ?>" name="<?php esc_attr_e($result->field_name); ?>" id="<?php esc_attr_e($result->field_name); ?>"><?php if ($result->field_name == 'post_content') esc_html_e($getad->post_content, 'colabsthemes'); else echo esc_html_e($post_meta_val, 'colabsthemes'); ?></textarea>
					<div class="clr"></div>
			...
			case 'radio':
				...
				<li id="list_<?php esc_attr_e($result->field_name); ?>">
					<div class="labelwrapper">
                    	<label><?php if ($result->field_tooltip) : ?><a href="#" tip="<?php esc_attr_e($result->field_tooltip); ?>" tabindex="999"><div class="helpico"></div></a><?php endif; ?><?php esc_html_e($result->field_label, 'colabsthemes'); ?>: <?php if ($result->field_req) echo '<span class="colour">*</span>'; ?></label>
					</div>
					...
					<ol class="radios">
						...
						?>
							<li>
								<input type="radio" name="<?php esc_attr_e($result->field_name); ?>" id="<?php esc_attr_e($result->field_name); ?>" value="<?php esc_html($option, 'colabsthemes'); ?>" class="radiolist <?php if ($result->field_req) echo 'required' ?>" <?php if ( trim($post_meta_val) == trim($option) ) echo 'checked="checked"'; ?>>&nbsp;&nbsp;<?php esc_html_e(trim($option), 'colabsthemes'); ?>
							</li> <!-- #radio-button -->
						<?php
						}
						?>
...
function colabs_associate_images($post_id,$file,$print = false) {
    ...
    for ($i=0; $i < count($file);$i++ ) {
        ...
        $attach_id = wp_insert_attachment( $attachment, $file[$i]['file'], $post_id );
				if($i==0)update_post_meta($post_id,'_thumbnail_id',$attach_id);
...
function colabs_paypal_notif() {

	if ((get_option('colabs_paypal_sandbox') == 'true')&&(current_user_can('manage_options'))){
    echo "<div class='error fade'><p>". sprintf( __('Classifier is currently running in PayPal Sandbox mode. Remember to <a href="%1$s">uncheck the box</a> when you are ready to start selling ads again.', 'colabsthemes'), 'admin.php?page=gateways#colabs-option-paypaloptions') ." </p></div>";
	}	
    
}
add_action('admin_notices', 'colabs_paypal_notif', 3);
?>